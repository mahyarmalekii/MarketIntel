...
const CRYPTO_TICKERS = ["BTC-USD", "ETH-USD", "SOL-USD", "BNB-USD", "XRP-USD", "ADA-USD", "DOGE-USD"];

export function CryptoView({ api }: { api: ApiFetch }) {
  ...
  useEffect(() => {
    loadQuotes(CRYPTO_TICKERS);
  }, [loadQuotes]);

  const addCustom = () => {
    if (customTicker) {
      loadQuotes([customTicker.toUpperCase()]);
      setCustomTicker("");
    }
  };
    ...
    <div className="ingestion-page scroll">
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px", display: "flex", flexDirection: "column", gap: 24 }}>
        <div className="card" style={{ padding: 22 }}>
          <div className="eyebrow" style={{ marginBottom: 14 }}>Crypto Markets</div>
          
          <div className="row gap-2" style={{ marginBottom: 20 }}>
            <input className="input" placeholder="e.g. AVAX-USD" 
                   value={customTicker} onChange={e => setCustomTicker(e.target.value)}
                   onKeyDown={e => e.key === "Enter" && addCustom()} />
            <button className="btn" onClick={addCustom}>Look up</button>
            <button className="btn btn-icon" onClick={() => loadQuotes(quotes.map(q => q.ticker) || CRYPTO_TICKERS)} disabled={loading}>
              {loading ? "..." : "Refresh"}
            </button>
          </div>
            ...
            {quotes.map(q => (
              <div key={q.ticker} style={{ background: "var(--paper-2)", borderRadius: 10, padding: 16, border: "1px solid var(--line)" }}>
                <div className="row" style={{ justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{ fontWeight: 700, fontSize: 16 }}>{q.ticker.replace("-USD", "")}</span>
                  <span className="mono" style={{ fontSize: 16, fontWeight: 600 }}>
                    ${q.last_price?.toLocaleString("en", { minimumFractionDigits: 2, maximumFractionDigits: 6 })}
                  </span>
                </div>
