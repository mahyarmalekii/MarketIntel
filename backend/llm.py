...
_FALLBACK_ORDER = ["anthropic", "openai", "gemini", "deepseek", "groq",
                   "mistral", "xai", "kimi", "ollama"]


def _call_raw_single(p: str, k: str, model: str, s: str, u: str) -> str:
    """Call a single provider synchronously; raises on failure."""
    if p == "anthropic":
        c = anthropic.Anthropic(api_key=k, timeout=120.0)
        r = c.messages.create(
            model=model, max_tokens=4096, system=s,
            messages=[{"role": "user", "content": u}],
        )
        return r.content[0].text
    elif p == "gemini":
...
def call_raw_with_fallback(s: str, u: str, step: str | None = None) -> str:
    """
    Same as call_raw but automatically tries the next configured provider
    if the primary one fails or has no key.
    """
    p, k, model = _resolve(step)
    ...
    if k or p == "ollama":
        try:
            return _call_raw_single(p, k, model, s, u)
        except Exception as exc:
            _log.warning("primary provider %s failed (%s) — trying fallback chain", p, exc)

    # Walk fallback order, skip primary, try any provider that has a key
    for fb_p in _FALLBACK_ORDER:
        if fb_p == p:
            continue
        fb_k = (
            get_setting(_KEY_NAMES.get(fb_p, ""), "")
            or os.environ.get(_ENV_NAMES.get(fb_p, ""), "")
            or (os.environ.get("GOOGLE_API_KEY", "") if fb_p == "gemini" else "")
        )
        if not fb_k and fb_p != "ollama":
            continue
        fb_model = get_setting(f"{fb_p}_model", _DEFAULT_MODELS.get(fb_p, ""))
        try:
            result = _call_raw_single(fb_p, fb_k, fb_model, s, u)
            _log.info("fallback to %s succeeded", fb_p)
            return result
        except Exception as exc:
            _log.warning("fallback %s also failed: %s", fb_p, exc)

    _log.error("all LLM providers exhausted — returning empty string")
    return ""
